...
In that case the scheduler reserves `min(max_new_tokens, --clip-max-new-tokens-estimation)` KV tokens for every new request (default 4096), which caps how many requests it admits at once regardless of `--schedule-conservativeness`. Lower `--clip-max-new-tokens-estimation` (e.g. 256) when outputs are usually short; the stop condition is unchanged and an under-estimate is handled by retraction.
