...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
      ...
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Type: float</td>
        ...
        <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}>`--clip-max-new-tokens-estimation`</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Clip the per-request max_new_tokens the scheduler uses when it estimates how many KV tokens a new request needs. Only the estimate is clipped; the stop condition is unchanged. Lower it when requests carry a large max_new_tokens but usually stop early, so more of them are admitted at once. Unset reads the deprecated `SGLANG_CLIP_MAX_NEW_TOKENS_ESTIMATION` env var.</td>
